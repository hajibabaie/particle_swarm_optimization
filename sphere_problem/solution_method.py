...
class PSO:
    ...
    def __init__(self,
        ...
        self._min_range_variables = min_range_of_variables
        self._max_range_variables = max_range_of_variables
        ...
        self._best_costs = []
        self._velocity_max = 0.1 * (self._max_range_variables - self._min_range_variables)
    ...
    def _initialization(self):
        ...
        for i in range(self._number_of_particles):
            ...
            self._particles[i].position = np.random.uniform(self._min_range_variables,
    ...
    def _update_velocity(self):
        ...
        for i in range(self._number_of_particles):
            ...
            np.random.uniform(0, 1, self._particles[i].velocity.shape) * self._personal_learning_rate * \
            (self._particles[i].position_best - self._particles[i].position) + \
            np.random.uniform(0, 1, self._particles[i].velocity.shape) * self._global_learning_rate * \
            ...
            self._particles[i].position = np.clip(self._particles[i].position,
                                                  self._min_range_variables, self._max_range_variables)
    ...
    def run(self):
        ...
        plt.title("Sphere Function Using Particle Swarm Optimization", fontweight="bold")
